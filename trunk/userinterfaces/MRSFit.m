...
if nargin > 0   % i.e. command comes from MRSWorkflow
    ...
    function gui = createInterface()
        ...
        if logplt
            for i=1:4
                set(gui.panel_sounding.snd(i),'yscale','log')
            end
            set(gui.panel_sounding.snd(2),'xscale','log')
        end
...
%             'Parent', box_p1v1h2);   
        ...
        gui.panel_controls.edit_v0fit = uicontrol(...
            ...
            'Parent', box_p2v1h2, ...
            'String', '-');
        gui.panel_controls.edit_v0er = uicontrol(...
            'Style', 'Edit', ...
            'Enable', 'on', ...
            'FontWeight', 'bold', ...
            'HorizontalAlignment', 'center', ...
        ...
        gui.panel_controls.edit_t2sfit = uicontrol(...
            ...
            'Parent', box_p2v1h3, ...
            'String', '-');
        gui.panel_controls.edit_t2ser = uicontrol(...
            'Style', 'Edit', ...
            'Enable', 'on', ...
            'FontWeight', 'bold', ...
            'HorizontalAlignment', 'center', ...
        ...
        gui.panel_controls.edit_dffit = uicontrol(...
            ...
            'Parent', box_p2v1h4, ...
            'String', '-');
        gui.panel_controls.edit_dfer = uicontrol(...
            'Style', 'Edit', ...
            'Enable', 'on', ...
            'FontWeight', 'bold', ...
            'HorizontalAlignment', 'center', ...
        ...
        gui.panel_controls.edit_phifit = uicontrol(...
            ...
            'Parent', box_p2v1h5, ...
            'String', '-');
        gui.panel_controls.edit_phier = uicontrol(...
            'Style', 'Edit', ...
            'Enable', 'on', ...
            'FontWeight', 'bold', ...
            'HorizontalAlignment', 'center', ...
...
%% MENU LOAD --------------------------------------------------------------
    function onLoad(a,call)
        ...
        for iQ = 1:length(proclog.Q)
            for irx = 1:length(proclog.Q(1).rx)
                tau_dead = proclog.Q(iQ).timing.tau_dead1;
                t = proclog.Q(iQ).rx(irx).sig(2).t + tau_dead;
                %std(proclog.Q(iQ).rx(irx).sig(2).V)/sqrt(length(proclog.Q(iQ).rx(irx).sig(2).V))
                proclog.Q(iQ).rx(irx).sig(2).V = proclog.Q(iQ).rx(irx).sig(2).V;
            end
        end
...
% Fit FID for currently selected Q & SIG
    function onFit(a,b)
        ...
        % Assemble parameters
        tau_dead = proclog.Q(iQ).timing.tau_dead1;
        t   = proclog.Q(iQ).rx(irx).sig(isig).t(it1:it2) + tau_dead;
...
%% FUNCTION PLOT DATA -------------------------------------------------
    function mrs_plotfit(gui,proclog)
        ...
        % assemble data
        tau_dead = proclog.Q(iQ).timing.tau_dead1;
        t  = proclog.Q(iQ).rx(irx).sig(isig).t(it1:it2) + tau_dead; % [s]
        ...
        switch isig
            case {2,3,4}
                ...
                plot(t,scalefactor_V*real(v)*-1, 'Color', col(isig,:))
                hold on
                plot(t,scalefactor_V*real(f)*-1, 'Color', 'r')
        ...
        %         ylim([-40 40])
                ...
                plot(t,scalefactor_V*imag(v)*-1, 'Color', col(isig,:))
                hold on
                plot(t,scalefactor_V*imag(f)*-1, 'Color', 'r')
                plot(t,zeros(size(t)), ':', 'Color', 3*[0.1 0.1 0.1])
                hold off
                xlim(xl)
        %         ylim([-40 40])
...
% update gui
    function refreshfitparameters
        ...
        if isfield(proclog.Q(iQ).rx(irx).sig(isig).fite,"ci")
            fite = proclog.Q(iQ).rx(irx).sig(isig).fite.ci;
        else
            fite = zeros(4,1);
        end
        ...
        switch isig
            case {2,3}
                ...
                set(gui.panel_controls.edit_dfmin,'string', num2str(ext_roundn(lb(3),-2)));   % [Hz]
                set(gui.panel_controls.edit_dfmax,'string', num2str(ext_roundn(ub(3),-2)));   % [Hz]
                ...
                set(gui.panel_controls.edit_dfini,'string', num2str(ext_roundn(ini(3),-2)));  % [Hz]
                ...
                set(gui.panel_controls.edit_v0er,'string', num2str(round(fite(1)*1e9,2,'significant')));  % [nV]
                set(gui.panel_controls.edit_t2sfit,'string', num2str(round(fitp(2)*1e3))); % [ms]
                set(gui.panel_controls.edit_t2ser,'string', num2str(round(fite(2)*1e3,2,'significant'))); % [ms]
                set(gui.panel_controls.edit_dffit,'string', num2str(ext_roundn(fitp(3),-2)));  % [Hz]
                set(gui.panel_controls.edit_dfer,'string', num2str(round(fite(3),2,'significant')));  % [Hz]
                set(gui.panel_controls.edit_phifit,'string', num2str(ext_roundn(fitp(4),-2))); % [rad]
                set(gui.panel_controls.edit_phier,'string', num2str(round(fite(4),2,'significant'))); % [rad]
            case {4}
    ...
    function fc = mrs_fitcorrections(f,iQ,isig)
        ...
        switch isig
            case 2  % RDP & DF correction for fid1
                ...
                td    = 0;% proclog.Q(iQ).timing.tau_dead1;  % trim not required because fit values f are wrt t=0. 
                fc(1) = mrs_RDPcorr(f(1),tp,td,f(2));
                %Tobias: IMO we should also account for the phase offset
                %caused by delays 
