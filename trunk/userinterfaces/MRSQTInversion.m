...
if nargin > 0   % i.e. command comes from MRSWorkflow
    ...
    function gui = createInterface()       
        ...
        set(gui.panel_controls.figureid, 'Position', [5 screensz(4)-800 350 735])
        ...
        uimenu(gui.FileMenu, 'Label', 'Load Data',   'Callback', {@onLoadData,0});
        uimenu(gui.FileMenu, 'Label', 'Add Data',   'Callback', {@onLoadData,1});
        uimenu(gui.FileMenu, 'Label', 'Load Kernel', 'Callback', {@onLoadKernel,0});
        uimenu(gui.FileMenu, 'Label', 'Add Kernel', 'Callback', {@onLoadKernel,1});
...
%             'Callback',@onPushbuttonGetInstrumentPhase);
        ...
        gui.para.modelDiscretisation = uicontrol('Style','Edit','String','0.5',...
                'parent',h4ModelSpace,'Visible','off','BackgroundColor', [1 1 1]);            
        ...
        gui.para.modelMaxDepth = uicontrol('Style','Edit','String','-','Visible','off',...
                'parent',h5ModelSpace,'BackgroundColor', [1 1 1]);            
...
%% FUNCTION onLoadData ----------------------------------------------------    
    function onLoadData(a,call,Add)
...
%         softwareversion = mrs_version;
        ...
        %                     idata.data.estack(m)  = proclog.Q(m).rx(1).sig(2).fite.E;
            ...
            case 1
                phase_input = inputdlg("data phase in °");
                phase = str2double(phase_input{1});
                switch iSig
                    case 2% FID
                        % check if there has been trim during fit and get indices
                        [minRecInd, maxRecInd] = mrs_gettrim(proclog,1,1,iSig);
                        assert(isequal(idata.data.tRaw,proclog.Q(1).rx(iChannel).sig(iSig).t(minRecInd:maxRecInd) - proclog.Q(1).rx(1).sig(iSig).t(minRecInd)),"Data have different time axis")
                        assert(isequal(idata.data.effDead,proclog.Q(1).timing.tau_dead1 +  proclog.Q(1).rx(iChannel).sig(iSig).t(minRecInd)),"Data have different time axis")
                        M = length(idata.data.q);
                        for m = 1:length(proclog.Q)
                            idata.data.q(M+m)       = proclog.Q(m).q;
                            % error estimation using the mono-fit (mrs_fitFID.m)
                            idata.data.efit(M+m)    = proclog.Q(m).rx(iChannel).sig(2).fite.E;
                            % error estimation using stacking (mrsSigPro_stack.m)
        %                     idata.data.estack(m)  = proclog.Q(m).rx(1).sig(2).fite.E;
                            idata.data.estack(M+m)  = real(mean(proclog.Q(m).rx(iChannel).sig(iSig).E));
                            idata.data.V0fit(M+m)   = proclog.Q(m).rx(iChannel).sig(iSig).fitc(1);
                            idata.data.T2sfit(M+m)  = proclog.Q(m).rx(iChannel).sig(iSig).fitc(2);
                            idata.data.df(M+m)      = proclog.Q(m).rx(iChannel).sig(iSig).fitc(3);
                            idata.data.phi(M+m)     = proclog.Q(m).rx(iChannel).sig(iSig).fit(4)+phase/180*pi;
                            idata.data.dcubeRaw(M+m,:) = proclog.Q(m).rx(iChannel).sig(iSig).V(minRecInd:maxRecInd).*factor*exp(-1i*phase/180*pi);
                        end
                    case 4 %T2
                        idata.data.dcubeRaw    = zeros(length(proclog.Q),proclog.Q(1).rx(iChannel).sig(iSig).nE);
                        % tRaw shold start with zero
        ...
        if isfield(idata,"bck")
            ...
            idata.bck.phi      = idata.data.phi;
            idata.bck.df       = idata.data.df;
            idata.bck.qK       = idata.kernel.q;
        ...
        gui = mrsInvQT_plotData(gui,idata);
        mrs_setguistatus(gui,0)
    end

%% FUNCTION onLoadKernel --------------------------------------------------
    function onLoadKernel(a,call,Add)
        if ~isnumeric(call)% workaround for matlab 2014b
            call=0;
        end
        ...
        switch Add
            case 0
                ...
                if isfield(in.kdata,"Kr")
                    idata.kernel.K = in.kdata.Kr;
                    idata.kernel.z    = in.kdata.model.r;
                    idata.kernel.Dz    = in.kdata.model.Dr;
                    %idata.kernel.z    = flip(0.35 - in.kdata.model.r);
                    idata.kernel.q    = in.kdata.measure.pm_vec';
                    idata.fn          = [];
                else
                    error("MRSTrees needs a radial Kernel (Kr)");
                end
            case 1
                phase_input = inputdlg("kernel phase in °");
                phase = str2double(phase_input{1});
                if isfield(in.kdata,"Kr")
                    idata.kernel.K    = [in.kdata.Kr*exp(-1i*phase/180*pi);idata.kernel.K];
                    assert(isequal(idata.kernel.z,in.kdata.model.r),"Kernels have different discretization")
                    assert(isequal(idata.kernel.Dz,in.kdata.model.Dr),"Kernels have different discretization")
                    idata.kernel.q    = [in.kdata.measure.pm_vec';idata.kernel.q];
                    idata.fn          = [];
                else
                    error("MRSTrees needs a radial Kernel (Kr)");
                end
        ...
        if isfield(idata,"bck")
            ...
            idata.data.phi      = idata.bck.phi;
            idata.data.df       = idata.bck.df;
        ...
        set(gui.para.modelDiscretisation,'String',num2str(min(idata.kernel.Dz)))
        set(gui.para.modelMaxDepth,'String',num2str(max(idata.kernel.z)))
        if size(idata.kernel.K,1) == size(idata.data.dcubeRaw,1)
            gui = mrsInvQT_plotData(gui,idata);
        end
        mrs_setguistatus(gui,0)
    end
...
%% RUN QTinversion
    function on_pushbuttonRun(a,b)
       ...
       switch idata.para.modelspace
           case 1 %smooth-multi
              if isfield(idata,"inv1Dqt") & isfield(idata.inv1Dqt,"smoothMulti") & isfield(idata.inv1Dqt.smoothMulti,"solution") 
                idata.inv1Dqt.smoothMulti = rmfield(idata.inv1Dqt.smoothMulti,"solution"); %remove previous runs
              end
              %idata = mrs_invQT1D(idata,1+idata.para.statisticRuns);
              idata = mrs_invQT1D_new(idata,1+idata.para.statisticRuns); 
              gui = mrsInvQT_plotData(gui,idata);
           case 2 %smooth-mono
              if isfield(idata,"inv1Dqt") & isfield(idata.inv1Dqt,"smoothMono") & isfield(idata.inv1Dqt.smoothMono,"solution") 
                idata.inv1Dqt.smoothMono = rmfield(idata.inv1Dqt.smoothMono,"solution"); %remove previous runs
              end
              %idata = mrs_invQT1DMono_new(idata,1+idata.para.statisticRuns);
...
%% some other function
    ...
    function onPushbuttonCheckQ(a,b)
        ...
        idata.data.phi      = idata.data.phi(idata.para.iQuse);
        idata.data.df       = idata.data.df(idata.para.iQuse);
        idata.kernel.K      = idata.kernel.K(idata.para.iQuse,:);
        idata.kernel.qK     = idata.kernel.q(idata.para.iQuse);
        % plot
        idata = mrs_invQTGetGuiPara(gui,idata);
        idata = mrs_invQTpreparation(idata);
        gui = mrsInvQT_plotData(gui,idata);
    ...
    function onSolType(a,b)
        ...
        switch idata.para.QTinv.soltype
            case 1
                ...
                set(gui.para.modelDiscretisationText,'visible','off','String','min. layer thickness/m');
                set(gui.para.modelMaxDepth,'Visible','off');
                set(gui.para.modelMaxDepthText,'Visible','off','String','max. depth/m');
    ...
    function onSaveData(a,call)
        ...
        if call
            ...
            [invfilename, path] = uiputfile(...
                ...
                [inifile.MRSData.path strrep(inifile.MRSData.file,'.mrsd','.mrsi')]);
        end
        % some parameter mapping
        idata.inv1Dqt.para = idata.para;
        outfile = fullfile(path,invfilename);
        save(outfile,'idata')
        set(gui.panel_controls.edit_invPath,'String',[path filesep invfilename]);
        fprintf(1,'inversion saved to %s\n', outfile); 
        mrs_updateinifile([path filesep invfilename],1);
        mrs_setguistatus(gui,0)   
    end
