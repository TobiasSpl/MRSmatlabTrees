...
%                            abs(transfer(freqSpec>freqLim(1) & freqSpec<freqLim(2))));
%                      end
            case 3 % Fixed --> constant transfer function
                transfer   = proclog.NC.rx(irx).sig(isig).TF;
                refChannel = proclog.NC.rx(irx).sig(isig).niref;
        end
%     case 'NUMISpoly'
...
if iQ == 1 & isfield(fdata,'noise')
    transReference=zeros(size(fdata.noise.rec(irec).rx(irx).sig(isig).v1));
    for nc=1:length(refChannel)
        transReference = transReference + ifft((transfer(:,nc).').*fft(fdata.noise.rec(irec).rx(refChannel(nc)).sig(isig).v1));
    end
    fdata.noise.rec(irec).rx(irx).sig(isig).v1 = fdata.noise.rec(irec).rx(irx).sig(isig).v1 - transReference;
end
