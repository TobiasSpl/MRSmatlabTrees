...
fdata.info.isnoise    = false;

tau_dead_extra = 0.002;
fdata.header.tau_dead = fdata.header.tau_dead + tau_dead_extra;
...
if chk ~= 2 % no info exist --> open request dialog
    ...
    fdata.info.listening = para{13,2}; %listening checkbox
    irx = 0;
    ...
    %check if listening info is in inp file
    info = textscan(fidinp,'#%s = %d');
    i=find(contains(info{1},'listening'));
    if isempty(i)
        fdata.info.listening = 0;
    else
        fdata.info.listening = info{2}(i);
    end
...
for irec = 1:length(fdata.header.fileID)
    ...
    data(irec)  = openGMRRawData(fdata, rec); 
    ...
    switch fdata.header.Qsampling
        case 0 % standard GMR, one file contains all pulse moments
            ...
            [~,index(irec,:)]  = sort(data(irec).q1); 
            q1 = [q1; data(irec).q1(index)];q1p = [q1p; data(irec).q1phase(index)];
            q2 = [q2; data(irec).q2(index)];q2p = [q2p; data(irec).q2phase(index)];
...
%this is the standart sort algorithm of the GMR Flex
nqleave = round(fdata.header.nQ/fdata.header.ileaves);
index_all = zeros(fdata.header.nQ,1);
for n=1:nqleave
    for ileave=1:fdata.header.ileaves
        iq = n+(ileave-1)*nqleave;
        i = find(index_all==0,1,"last");
        index_all(i) = iq;
    end
end

%check if assumed sequence of qs is roughly in the same order as detected sequence of qs, otherwise sort by hand

if ~sum(index(:,:)'==index_all,"all") > size(index,1)*size(index,2)/2
    warning("GMR q-sorting is wrong")
    %we sort the indices of all measurements together, starting at the top of each interleave
    for n=1:nqleave
        for ileave=1:fdata.header.ileaves
            iq = n+(ileave-1)*nqleave;
            %[~,i] = max(sum(index==iq)); %bias to small index
            i = find(sum(index==iq,1)==max(sum(index==iq,1)),1,"last");
            if index_all(i) > 0
                i = find(sum(index,1) > 0,1,"last");
            end
            index_all(i) = iq;
            index(:,i) = 0; %remove used column
            %index(index==iq) = 0; %remove entries of iq that might be in other columns
        end
    end
end

for irec = 1:length(fdata.header.fileID)
    for n=1:length(data(irec).q1)
        fdata = GMRRawData2MRSmatlab(data(irec),fdata,index_all(n),irec,n);
    end
end

mrs_setguistatus(gui,0)
fdata.header.fS_org = fdata.header.fS; %save original sampling freq.
...
if chk ~= 2 % inp file already exists no rewrite
    ...
    fprintf(inpfile, '#listening = %d \n', fdata.info.listening); %save listening info to inp file
...
fdata.info.reducedQ = 1; %this is the unreduced dataset with all qs

if fdata.info.listening
    fdata = mrsSigPro_reduceQ(fdata,fdata.header.nQ); %reduce to one q and nq*nrec records
end
