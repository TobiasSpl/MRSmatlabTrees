function [fitpar,errorpar] = mrs_fitFID(t,v,lb,ini,ub)
...
switch cft
    ...
    case 0 % Curve fitting toolbox not available
        ...
        [fitpar1,residual1,jacobian] = mrs_MonoComplexExpFitCGLSCDP(v*sscale,t,ini,lb,ub);
        ...
        ini(1) = 10^p(2)*sscale;
        ini(2) = -1/p(1);
        [fitpar2,residual2,jacobian] = mrs_MonoComplexExpFitCGLSCDP(v*sscale,t,ini,lb,ub);
...
% the NEGATIVE sine.
    realpart =  x(1)*exp(-t/x(2)).*cos(2*pi*x(3)*t+x(4));
