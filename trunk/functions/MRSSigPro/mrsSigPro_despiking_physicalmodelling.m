...
%}
%modified for new filters, not tested
hv          = mrs_hilbert(v);
ehv         = hv.*exp(-1i*2*pi*fT.*t);
V           = filtfilt(proclog.LPfilter,ehv);
%sig         = real(V);
...
V2    = filtfilt(proclog.LPfilter,ehv2);
