...
fcontent   = textscan(fidh, hform, 1);     % read 11 header entr0ies (11 is the minimum # of entries for all DAQ versions)
DAQversion = ext_roundn(fcontent{10},-2);

% exit if DAQ version is not supported
supported  = [0.00 1.00 2.00 2.08 2.40 2.50 2.90 2.93 2.95 2.97 2.99 3.13 3.14 3.15 3.16];
if ~ismember(DAQversion,supported)
    msgbox('DAQ version not supported')
...
%        msgbox('DAQ version 2.99 has error in header! expUni is set to 4 Ch. To change edit openGMRheader line165') 
    ...
    case {3.13 3.14}  
        % reread header
        fseek(fidh,0,'bof');                % return to begin of header file
        hform    = repmat(hform1,1,78);     % 78 header entries in vers = 3.14 check for 3.13
        fcontent = textscan(fidh, hform, 1); 
        fclose(fidh);

        % extract info
        header.ileaves    = fcontent{11};       % 11	# Interleaves   1-3
        header.tau_dead   = fcontent{12}*1e-3; % 12	rx Dead time (ms)!
        %header.tau_dead   = 8*1e-3;             % 12	check later
        header.Qsampling  = fcontent{13};       % 13	Pulse Moment Sampling Method	0: Auto q-sampling; 1: Custom q-sampling (0)
        header.preampgain = fcontent{14};       % 14	Preamp Gain	500; 
        header.fS         = fcontent{15};
        header.expUnt     = fcontent{16};
...
%        msgbox('DAQ version 2.99 has error in header! expUni is set to 4 Ch. To change edit openGMRheader line165') 
        ...
        header.PPDelay = fcontent{72}*1e-3;
    case {3.15 3.16}  
        % reread header
        fseek(fidh,0,'bof');                % return to begin of header file
        hform    = repmat(hform1,1,78);     % 78 header entries in vers > 3.146
        fcontent = textscan(fidh, hform, 1); 
        fclose(fidh);
...
switch header.TXversion    % Transmitter version (defines Tx & Rx gains)
    ...
    case 4
        header.gain_I = 150;
    case {12}
        header.gain_I = 125; % new version with GMR Flex: check if this is correct!
    otherwise
...
switch header.TXversion
    case{12} %GMR Flex
        C      = header.tuning_C;
        if C==0
            fprintf('Warning: tunning capacity is zero, therefore circuit gain is assumed to be 1. This might be a bad assumtion for high inductance layouts\n')
            header.circuit_gain=1; %set circuit gain to 1 for Flex without tunning
        else
            R_coil = 1;         %resistance of nmr detection coil (Ohms), assumed value. significant change to circuit gain for R_coil > 5 Ohms
            w_nmr = 2*pi*header.fT;
            L_coil = 1/(C*(w_nmr^2));
            %L_coil = 0.0039;
            %calculate input impedance of preamp
            Rs = 41570;         %damping resistance on transformer secondary (Ohms) 250k||50k lower input impedance setting
            %Rs = 250000;       %damping resistance on transformer secondary (Ohms)
            Cp = .0000001;      %low pass filter capacitance added to primary side of transformer (F)
            Rp = Rs/(24^2);     %damping resistance reflected to primary side of transformer (Ohms)
            R_primary = 1.4;    %resistance of primary winding on Jensen preamp (Ohms)
            L_primary = 0.4;    %inductance of primary winding on Jensen preamp (H)
            Zp_inv = (1/Rp)+(1i*w_nmr*(Cp+.000000021))+(1/(R_primary+1i*w_nmr*L_primary));   %input capacitance of Jensen preamp is 21nF
            Zp = 1./(Zp_inv);
            Z_coil = 1i*w_nmr*L_coil+R_coil;
            Z_total = Z_coil+Zp;
            Io = 1/Z_total;
            header.circuit_gain = abs(Io*Zp);
            %fprintf('circuit gain = %.4f, should be close to 1\n', header.circuit_gain) %print to check
        end
    otherwise %normal GMR
        C      = header.tuning_C;
        assert(C>0,'tunning capacity can not be 0 for GMR')
        wT     = 2*pi*header.fT;
        L      = 1/(C*wT^2);
        Z1     = 0.5 + 1i*0.5*wT;
