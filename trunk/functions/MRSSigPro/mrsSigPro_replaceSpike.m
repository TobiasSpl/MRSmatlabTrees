...
switch isig
    case {2} %drops measurements of different phase from despike 
        if mod(irec,2) == 0
            rec(irec) = [];
            rec(mod(rec,2) == 1) = [];
        else
    ...
    case {4} %case {2,4} % drops paired measurement from despike % phase cycle for p90 is relevant
...
v_all  = zeros(fdata.header.nrec,length(t1)); % Tobias: error for single rec (listening), size of v_all must be the total number of stacks, previous version: v_all  = zeros(length(rec),length(t1)); use v1 instead of v0 to include previous processing steps
...
if threshold == -1 % manual
    ...
    fixme = round((( max([cutcenter-width/2,t1(2)]) : dt : ...   % t(2)/dt -> index 2 because first sample is 1 and not 0. (?)
                    min([cutcenter+width/2,max(t1)]) )-t1(1))/dt);     % max / min to prevent window being larger than timeserie    
    ...
    switch type
        ...
        case 1 % 'q stack'
            ...
            for iirec = rec
                ...
                v_all(iirec,:) = filtfilt(proclog.LPfilter,ehv);
                if strcmp(fdata.info.device,'GMR')
