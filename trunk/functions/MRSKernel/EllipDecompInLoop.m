...
B_senk(:,:,1)      = B1.x - skalarprodukt .* B0.x;
B_senk(:,:,2)      = B1.y - skalarprodukt .* B0.y;
B_senk(:,:,3)      = B1.z - skalarprodukt .* B0.z;

% Berechnung Parallelkomponente
B_par(:,:,1) = skalarprodukt .* B0.x;
B_par(:,:,2) = skalarprodukt .* B0.y;
B_par(:,:,3) = skalarprodukt .* B0.z;

% Berechnung der Vorzeichenfunktion
VZcross_1     = B0.x .* (B_senk(:,:,2) .* conj(B_senk(:,:,3)) - B_senk(:,:,3) .* conj(B_senk(:,:,2)));
VZcross_2     = B0.y .* (B_senk(:,:,3) .* conj(B_senk(:,:,1)) - B_senk(:,:,1) .* conj(B_senk(:,:,3)));
VZcross_3     = B0.z .* (B_senk(:,:,1) .* conj(B_senk(:,:,2)) - B_senk(:,:,2) .* conj(B_senk(:,:,1)));
...
Bs_mal_konjBs = B_senk(:,:,1) .* conj(B_senk(:,:,1)) + B_senk(:,:,2) .* conj(B_senk(:,:,2)) + B_senk(:,:,3) .* conj(B_senk(:,:,3));
Bs_mal_Bs     = B_senk(:,:,1).^2+B_senk(:,:,2).^2+B_senk(:,:,3).^2;
...
B_comps.zeta = angle(B_comps.e_zeta);

% Berechnung des Einheitsvektors bt ---------------------------------------
B_comps.b_1 = 1./B_comps.alpha .* real((1./B_comps.e_zeta) .* B_senk(:,:,1));
B_comps.b_2 = 1./B_comps.alpha .* real((1./B_comps.e_zeta) .* B_senk(:,:,2));
B_comps.b_3 = 1./B_comps.alpha .* real((1./B_comps.e_zeta) .* B_senk(:,:,3));

B_comps.bpar = skalarprodukt;

B_comps.bpar1 = B_par(:,:,1);
B_comps.bpar2 = B_par(:,:,2);
B_comps.bpar3 = B_par(:,:,3);

B_comps.s1=B_senk(:,:,1);
B_comps.s2=B_senk(:,:,2);
B_comps.s3=B_senk(:,:,3);
B_comps.s =sum(B_senk,3);

B_comps.x = B1.x;
B_comps.y = B1.y;
B_comps.z = B1.z;

if ~isreal(B_comps.alpha)
    disp('take care B_comps.alpha is imaginary but should not!')
    B_comps.alpha = real(B_comps.alpha);
end
if ~isreal(B_comps.beta)
    if ~isreal(B_comps.beta(real(B_comps.beta)~=0)) %there is a negligible imaginary component (<1E-15) when the real part is zero
