...
if numel(phi) == 1
    ...
    % assemble rotation matrix R
    R(1,1,otherdims{:}) = nx.*nx.*omcos +     cosp;
    R(1,2,otherdims{:}) = nx.*ny.*omcos - nz.*sinp;
    R(1,3,otherdims{:}) = nx.*nz.*omcos + ny.*sinp;
    
    R(2,1,otherdims{:}) = ny.*nx.*omcos + nz.*sinp;
    R(2,2,otherdims{:}) = ny.*ny.*omcos +     cosp;
    R(2,3,otherdims{:}) = ny.*nz.*omcos - nx.*sinp;
    
    R(3,1,otherdims{:}) = nz.*nx.*omcos - ny.*sinp;
    R(3,2,otherdims{:}) = nz.*ny.*omcos + nx.*sinp;
    R(3,3,otherdims{:}) = nz.*nz.*omcos +     cosp; 
end
