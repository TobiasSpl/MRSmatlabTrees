...
data.gammaH             = 267522189.96689254;
data.K                  = [];
data.mes_conf           = 1;
...
data.loop.size          = 1; % diameter for circular, side for square
data.loop.turns         = 1; % number of turns [transmitter]
data.loop.Rxsize        = 1; % diameter for circular, side for square
data.loop.Rxturns       = 10; % number of turns [receiver]
data.loop.rmax          = 300; 
data.loop.dr            = 24;
data.loop.PXsize        = 1;
data.loop.PXshape       = 1;
...
data.loop.dipolePosition= [0 0 -0.05];
...
data.earth.res          = 1; % 1: earth is a insulator, i.e. any conductivity is ignored; 0 - consider conductivity 
data.earth.nl           = 1;
data.earth.zm           = []; % Depth
...
data.model.dh           =        1;   %
data.model.dphi         = 1*pi/180;   % for pol coordinates   [deg]
...
data.measure.K3Dzphir    = 1;
data.measure.K3Dslices   = 0;
...
data.measure.applyBS      = 0; % apply Bloch Siegert shift correction 0/1
...
data.measure.fmod.shape    = 1; % shape of freq-modulation; 1: const., 2: linear; 3: tanh
...
data.measure.fmod.A        = 3; % parameter to describe shape of freq-modulation
data.measure.fmod.B        = 0; % parameter to describe shape of freq-modulation
...
data.measure.Imod.Qdf      = 0; % offset between Eigen-freq of circuit and Larmor freq
data.measure.Imod.Qf0      = data.earth.f+data.measure.Imod.Qdf; % Eigen-freq of tuned circuit 
...
data.measure.PX            = 0; % pre-polarization
data.measure.pulsesign     = 1; % standart: only adiabatic component of M0, +y pulse

data.measure.applyBS = 1;
data.measure.usePxFIDcorr = 0;

data.measure.useTxLookup = false;
