...
colors = ["#0072BD","#D95319","#EDB120","#7E2F8E","#77AC30","#4DBEEE","#A2142F","#FF0000","#00FF00","#0000FF","#00FFFF","#FF00FF","#FFFF00","#000000","#FFFFFF"];

%% inversion
disp('start inversion ...'); 
% primary run on measured data
phaseI=[];
reTQ = 1:length(idata.data.dcube(:));
runCGLSCDP(1);               
...
for sRun=2:statisticRuns
    ...
    reTQ  = sort(mrs_randsample(1:length(idata.data.dcube(:)),length(idata.data.dcube(:)),'true'));
    ...
    dcube = reshape(idata.data.dcube(reTQ),size(idata.data.dcube));
    ecube = reshape(idata.data.ecube(reTQ),size(idata.data.ecube));
    
    switch idata.para.dataType
        case 1
            D = reshape(abs(dcube),size(dcube,1)*length(idata.data.t),1);
            E = 1/reshape(1./abs(ecube),size(ecube,1)*length(idata.data.t),1);
        case 2
            D = reshape(real(dcube),size(dcube,1)*length(idata.data.t),1);
            E = reshape(1./real(ecube),size(ecube,1)*length(idata.data.t),1);
        case 3
            D = reshape(dcube,size(dcube,1)*length(idata.data.t),1);
            E = reshape(1./ecube,size(ecube,1)*length(idata.data.t),1);
    end
    runCGLSCDP(sRun);
...
%% some nested functions due to use memory efficient
    ...
    function runCGLSCDP(run) 
        ...
        if run>1
            ...
            %w            = w_start*ones(length(idata.inv1Dqt.smoothMono.z),1); 
            %T2           = T2_start*ones(length(idata.inv1Dqt.smoothMono.z),1);
        end
...
%                     TMPdD     = [real(D)-real(TMPd_est); imag(D)-imag(TMPd_est)]; 
            ...
            idata.inv1Dqt.smoothMono.solution(run).mnormW = norm(LW*w);
            idata.inv1Dqt.smoothMono.solution(run).mnormT2 = norm(LT*T2);
            mrsInvQT_plotData(tmpgui,idata,run);
            color = circshift(colors,-run);
            set(0,'currentFigure',tmpgui.fig_misfit);plot(iter,dnorm,'o',"Color",color(1)); xlabel('iteration');ylabel('misfit');grid on; hold on;
            ...
            if iter == 4
                threshold =  (dnorm_old-dnorm)/100;
            end
            if iter > 8 & dnorm_old-dnorm < threshold
                break
            end
        end
