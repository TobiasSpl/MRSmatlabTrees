function idata = mrs_invQT1D(idata,statisticRuns)
...
switch idata.para.dataType
    case {1,2}
        D      = idata.data.d;
    case {3}
        D      = abs(idata.data.d) .* exp(1i*(angle(idata.data.d) - idata.para.instPhase));
end
...
nT2 = size(decayT,2);
for iz = 1:nz
    %Layers(iz,(iz-1)*nT2+1:iz*nT2) = 1;
    Layers(iz,iz:nz:nT2*nz) = 1;
end


%% smoothness
[Lx,Ly]       = get_l2D(size(g,2),size(decayT,2));
%L             = idata.para.regMonoWC/idata.para.regMonoT2*(Lx) + Ly;
L             = idata.para.regMonoWC/idata.para.regMonoT2*(Lx'*Lx) + Ly'*Ly;
M0            = 0.01/size(timeT,2)*ones(size(g,2)*size(decayT,2),1);
%M0            = 0.0001/size(timeT,2)*ones(size(g,2)*size(decayT,2),1);
...
E_backup = E;
G_backup = G;
for k = 2:statisticRuns
as(k) = idata.para.regMonoT2;
fprintf("run %d / %d\n",k,statisticRuns)
reTQ  = sort(mrs_randsample(1:length(D),length(D),'true'));
D = D_backup(reTQ);
E = E_backup(reTQ);
G = G_backup(reTQ,:);
runCGLSCDP;
...
%% some nested functions due to use memory efficient
    ...
    function M_est = runCGLSCDP 
        ...
        lam          = as(k);
        M_est_old    = M0;
        %M_est_old    = lowestT2(M_est_old);
        %M_est_old    = reduceT2(M_est_old);
        M_est        = M0;
        %M_est        = lowestT2(M_est);
        %M_est        = reduceT2(M_est);
        [A,b,e]      = transform; %[J,dD,e]
        dM           = cglscdp;
        %dM           = reduceT2(dM);
        M_est        = linesearch;
        M_est        = upperbound;
        ...
        idata.inv1Dqt.smoothMulti.solution(k).w       = sum(reshape(M_est,size(g,2),size(decayT,2)),2);
        switch idata.para.dataType
            case {1,2}
                idata.inv1Dqt.smoothMulti.solution(k).dnorm    = norm(E.*(D - abs(G*M_est)))/sqrt(length(D));
            case {3} %D is complex, therefore no abs; number of observations is doupled
                idata.inv1Dqt.smoothMulti.solution(k).dnorm    = norm(E.*abs(D - G*M_est))/sqrt(length(D)*2);
        end
        ...
        switch idata.para.dataType
            ...
            case {3} %D is complex, therefore no abs; number of observations is doupled
                deltaData(n)  = norm(E.*abs(D - (G*M_est)))/sqrt(length(D)*2);
        end
        ...
        while deltaModel(n) > dMmin
            ...
            idata.inv1Dqt.smoothMulti.solution(k).mnorm    = norm(L*M_est);
            idata.inv1Dqt.smoothMulti.solution(k).mnormT2    = norm(Ly*M_est);
            idata.inv1Dqt.smoothMulti.solution(k).mnormW    = norm(Lx*M_est);
            mrsInvQT_plotData(tmpgui,idata);
        ...
        function M = reduceT2(M_est)
            T2small = 10E-3;
            reduce = decayT<T2small;
            M_tmp    = reshape(M_est,size(g,2),size(decayT,2));
            M_tmp(:,reduce) = M_tmp(:,reduce).*repmat(exp(-0.5*log((decayT(reduce)/T2small)).^2),size(g,2),1);
            M = reshape(M_tmp,size(G,2),1);
        end

        function M = lowestT2(M_est)
            T2low = 1E-3;
            setZero = decayT<T2low;
            M_tmp    = reshape(M_est,size(g,2),size(decayT,2));
            M_tmp(:,setZero) = 0.0;
            M = reshape(M_tmp,size(G,2),1);
        end
